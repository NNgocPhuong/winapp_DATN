...
namespace System
{
    public class StringTime
    {
        ...
        public string Date => text.Substring(0, 10);

        public string Time => text.Substring(11);

        public string YearMonth => text.Substring(0, 7);

        public string Year => text.Substring(0, 4);

        public string Month => text.Substring(5, 2);

        public string Day => text.Substring(8, 2);

        public string Hour => text.Substring(11, 2);

        public string Minute => text.Substring(14, 2);

        public string Second => text.Substring(17, 2);

        public StringTime()
        {
        }

        public StringTime(DateTime time)
        {
            text = time.ToString("yyyy-MM-dd HH:mm:ss");
        }

        public StringTime(string value)
        {
            text = value;
        }
        ...
        public int CompareTo(StringTime time)
        {
            return text.CompareTo(time.text);
        }

        public static implicit operator StringTime(string str)
        {
            return new StringTime(str);
        }

        public int GetNibble(int index)
        {
            return (byte)(Ticks >> ((byte)index << 2)) & 0xF;
        }
    }
}